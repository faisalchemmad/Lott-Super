...
class GameResultViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Check if result already exists for this game/date/state
        existing = GameResult.objects.filter(game_id=game_id, date=date, state=state).first()
        if existing:
            # Re-route to update
    ...
    def perform_destroy(self, instance):
        # Reset bets associated with this game/date/state
        game = instance.game
        date = instance.date
        res_state = instance.state
        Bet.objects.filter(game=game, state=res_state, created_at__date=date).update(
            is_winner=False,
            winning_amount=0,
        ...
        from .models import ForwardedBet
        ForwardedBet.objects.filter(game=game, state=res_state, date=date).update(
            is_winner=False,
            winning_amount=0
        )
        instance.delete()
    ...
    def _calculate_winners(self, game_result):
        ...
        # 2. Reset bets for this game/date/state
